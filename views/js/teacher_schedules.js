$(document).ready(function () {
  ...
  function getAllData() {
    sampleArray = [];
    $.get({
      url: "../controllers/teacher_schedules/teacher_schedulesCrud.php",
      data: { getData: "getData" },
      success: function (data) {
        let newData = JSON.parse(data);
        let table = $("#bamsmsTable");
        newData.forEach((teacher_schedules, i) => {
          sampleArray.push(teacher_schedules);
          let tableRow = $("<tr>", { id: newData.id });
          $("<td>", { class: "text-wrap", html: i + 1 }).appendTo(tableRow);
          $("<td>", {
            class: "text-wrap",
            html:
              teacher_schedules.fname +
              " " +
              teacher_schedules.mname +
              " " +
              teacher_schedules.lname,
          }).appendTo(tableRow);

          let sched_day = JSON.parse(teacher_schedules.sched_day);
          let day = "";
          sched_day.forEach((days, i) => {
            if (i == sched_day.length - 1) {
              day += days;
            } else {
              day += days + ",";
            }
          ...
          $("<td>", {
            class: "text-wrap",
            html: teacher_schedules.prefered_time_from,
          }).appendTo(tableRow);

          $("<td>", {
            class: "text-wrap",
            html: teacher_schedules.prefered_time_to,
          }).appendTo(tableRow);
  ...
  $("#add-new").click(function () {
    ...
    $("#modalMainLabel").html("Add Teacher Schedule");
  ...
  function update(index) {
    $("#modalMain").modal("show");
    $("#modalMainLabel").html("Update Teacher Schedule");
  ...
  function view(index) {
    $("#modalMain").modal("show");
    $("#modalMainLabel").html("View Teacher Schedule");
    let models = sampleArray[index];
    Object.keys(models).map((key) => {
